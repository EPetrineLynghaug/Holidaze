...
const Section = ({ icon, title, children }) => (
  <section className="bg-white shadow-md rounded-xl w-full max-w-6xl p-6 md:p-8 space-y-6 ring-1 ring-gray-200 text-left">
...
export default function MyBookingsDashboardDesktop() {
  ...
  const { bookings, loading, error, refetch } = useBookings(userName);
    ...
    <main className="w-full max-w-7xl mx-auto max-h-screen overflow-y-auto px-6">
      ...
      <div className="flex gap-4 mt-6">
        {FILTERS.map((f) => (
          ...
          >
            {f.label} (
            {f.key === "upcoming" ? upcomingList.length : pastList.length})
          </button>
        ))}
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
          ...
          <div
            className={`md:col-span-6 space-y-6 ${
              !current ? "flex items-center justify-center text-gray-500" : ""
            }`}
          >
            ...
            {current && (
              <Section
                ...
                title={
                  filter === "upcoming" ? "Upcoming Booking" : "Past Booking"
                }
              >
                <img
