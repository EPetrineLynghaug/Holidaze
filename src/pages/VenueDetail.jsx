...
const formatNOK = (n) => {
  const formatted = new Intl.NumberFormat("nb-NO", { minimumFractionDigits: 0 }).format(n);
...
export default function VenueDetail() {
  ...
  return (
    ...
    ">
      ...
      {showSuccess && (
        <BookingSuccessPopup
          onClose={() => { setShowSuccess(false); navigate("/profile"); }}
        />
      )}
      ...
      {errMsg && (
        <p className="fixed bottom-20 inset-x-0 text-center text-red-500 text-xs">{errMsg}</p>
      )}
    </div>
  );
}
