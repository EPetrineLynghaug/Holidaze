...
export default function MyVenuesDashboardDesktop() {
  ...
  // Fetch venues with booking details
  useEffect(() => {
  ...
  const activeList = venues.filter(v => !isExpired(v));
  const expiredList = venues.filter(v => isExpired(v));
  const displayed = filter === 'active' ? activeList : expiredList;
  ...
  return (
    <div className="w-full max-w-7xl mx-auto mt-12 mb-20 p-6 md:p-8 bg-gray-50 rounded-2xl max-h-[calc(100vh-200px)] overflow-y-auto">
      ...
      {/* Wrapper for extra info if needed */}
      <div className="mb-6">
        {/* This extra div can fetch or display more summary info */}
      </div>

      {/* Venue List using your card component */}
      {loading ? (
