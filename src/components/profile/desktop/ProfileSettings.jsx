...
const Section = ({ icon, title, children }) => (
  <section className="bg-white shadow rounded-lg w-lvh max-w-6xl p-6 md:p-8 space-y-6 ring-1 ring-gray-100 text-left">
...
export default function ProfileSettingsPage({ userName, onSave }) {
  ...
  return (
    <main className="w-full max-w-none ml-0% max-h-screen overflow-y-auto">
      <form
        onSubmit={onSubmit}
        className="mt-10 mb-20 px-96 max-h-screen overflow-y-auto space-y-8"
      >
          ...
          <div className="flex items-center justify-between w-full">
            ...
            <Switch
              ...
              className={`inline-flex items-center h-8 w-16 rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-purple-500 ${
                profile.venueManager ? "bg-purple-600" : "bg-gray-300"
              }`}
            >
              ...
              <span
                className={`inline-block h-6 w-6 bg-white rounded-full shadow transform transition-transform ${
                  profile.venueManager ? "translate-x-8" : "translate-x-1"
                }`}
              />
            </Switch>
          </div>
