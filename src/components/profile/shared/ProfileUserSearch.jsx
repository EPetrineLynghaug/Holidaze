...
export default function VenueSearchSlide({ className = "" }) {
  ...
  const { profiles, venues } = useMemo(() => {
    ...
    if (Array.isArray(data)) {
      data.forEach(item => {
        if (item && item.type === "profile") ps.push(item);
        else if (item) vs.push(item);
      });
            ...
            {tabs.map(tab => (
              <button
                ...
                className={`flex-1 py-2 text-center text-xs md:text-sm font-semibold tracking-wide transition-colors duration-200
                  ${activeTab === tab.key ? "text-indigo-600 bg-white border-b-2 border-indigo-600" : "text-gray-500 hover:text-indigo-600 bg-gray-50"}`}
              >
                {tab.label}
              </button>
            ))}
            ...
            {!loading && !error && (
              <>
                {(activeTab === "all" || activeTab === "profiles") && (
                  <ul role="listbox" aria-label="Profile results" className="divide-y divide-gray-100">
                    {displayedProfiles.length > 0 ? (
                      displayedProfiles.map(p => (
                        <li key={p.id} role="option" className="flex items-center gap-3 px-4 py-2 cursor-pointer hover:bg-indigo-50 transition">
                          <Link
                          ...
                          >
                            ...
                            />
                            <span className="font-normal text-sm truncate text-gray-900">{p.displayName || p.name}</span>
                          </Link>
                        </li>
                      ))
                    ) : (
                      <li className="px-4 py-2 text-xs text-gray-400">No profiles found.</li>
                    )}
                  </ul>
                )}

                {(activeTab === "all" || activeTab === "venues") && (
                  <ul role="listbox" aria-label="Venue results" className="divide-y divide-gray-100">
                    {displayedVenues.length > 0 ? (
                      displayedVenues.map(v => (
                        <li key={v.id} role="option" className="px-4 py-2 cursor-pointer hover:bg-indigo-50 transition">
                          <Link
                            to={`/venues/${v.id}`}
