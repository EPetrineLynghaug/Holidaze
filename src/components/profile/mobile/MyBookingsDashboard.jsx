...
export default function MyBookingsDashboard({ onClose }) {
  ...
  const handleRating = (bid, aspect, value) => setReviews(prev => ({
    ...prev,
    [bid]: { ...prev[bid], [aspect]: value }
  }));
  const handleComment = (bid, text) => setReviews(prev => ({
    ...prev,
    [bid]: { ...prev[bid], comment: text }
  }));
  const handleSubmit = bid => console.log('Submit review', bid, reviews[bid]);
          ...
          {displayed.map(b => (
            <li key={b.id}>
              <button
                ...
                className={`w-full text-left p-3 rounded-lg ${selectedId===b.id ? 'bg-indigo-100' : 'bg-white hover:bg-gray-50'} shadow-sm flex justify-between items-center`}
              >
      ...
      {selected && filter === 'upcoming' && (
        <div className="bg-white rounded-2xl shadow-md p-6 space-y-2">
          <p className="text-gray-600">
            Upcoming from <strong>{new Date(selected.dateFrom).toLocaleDateString()}</strong> to <strong>{new Date(selected.dateTo).toLocaleDateString()}</strong>
          </p>
          <button onClick={() => handleCancel(selected.id)} className="text-red-500 font-medium hover:underline">Cancel Booking</button>
        </div>
      )}

      {selected && filter === 'expired' && (
        <div className="bg-white rounded-2xl shadow-md p-6 space-y-4">
          <h3 className="text-lg font-semibold">Review your stay at <span className="text-indigo-600">{selected.venue?.name}</span></h3>
          <div className="space-y-3">
            {aspects.map(a => (
              <div key={a} className="flex items-center">
                <span className="w-24 text-sm font-medium text-gray-700">{a}</span>
                <div className="flex space-x-1">
                  {[1,2,3,4,5].map(i => (
                    <span
                      key={i}
                      className={`material-symbols-outlined icon-gray ${review[a] >= i ? 'icon-purple filled' : ''} cursor-pointer hover:scale-110 active:scale-90 transition-transform`}
                      onClick={() => handleRating(selected.id, a, i)}
                    >{review[a] >= i ? 'star' : 'star_outline'}</span>
                  ))}
                </div>
              </div>
            ))}
          ...
          />
          <button onClick={() => handleSubmit(selected.id)} className="w-full bg-indigo-600 text-white py-2 rounded-lg hover:bg-indigo-700">Submit Review</button>
        </div>
      )}
    </div>
  );
